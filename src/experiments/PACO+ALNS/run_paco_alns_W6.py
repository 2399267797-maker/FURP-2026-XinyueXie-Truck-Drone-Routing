"""
PACO+ALNS v2.2 — Standalone experiment runner (adaptive parameter scaling).

Runs CollaborativePACOALNS (v2.2) on all Solomon RC configurations
(25c/50c/100c, RC1/RC2, medium/high endurance) and saves results,
Pareto plots, and route visualizations to results/20260719/.

Usage:
    python run_paco_alns_W6.py                    # full 12-config experiment
    python run_paco_alns_W6.py --size small       # 25c only (4 configs)
    python run_paco_alns_W6.py --size large       # 100c only (4 configs)
    python run_paco_alns_W6.py --runs 3           # quick test
"""
import os
import sys
import json
...
# ── Load PACO+ALNS ──────────────────────────────────────────────────────────
alns_mod_path = os.path.join(BASE, 'PACO+ALNSW6.py')
...
def run_experiment(model: VRPTruckDroneModel, n_runs: int = 10) -> Dict:
    """Multi-run experiment for PACO+ALNS v2.2."""
    print("\n--- PACO+ALNS v2.2 ---")
...
def plot_pareto(result: Dict, save_path: str):
    """Plot Pareto front of PACO+ALNS v2.2."""
    ...
    plt.scatter(pf_c, pf_t, facecolors='none', edgecolors='#d62728',
                marker='^', s=60, label='PACO+ALNS v2.2')
    ...
    plt.title('PACO+ALNS v2.2 — Non-dominated Front', fontsize=14, fontweight='bold')
...
def main():
    ...
    results_dir = os.path.join(BASE, 'results', '20260727')
    ...
    for cfg in configs:
        ...
        print(f"\n{'='*60}")
        print(f"PACO+ALNS v2.2 | {n}c | {rc} | {nv}V | {end}")
        print(f"{'='*60}")
        ...
        plot_pareto(result, os.path.join(results_dir, f'pareto_w6_{exp_key}.png'))

        # Route plots (min cost, min tardiness, compromise)
        n_trucks = model.get_number_of_trucks()
        n_drones = model.get_number_of_drones()
        title = f'PACO+ALNS v2.2 | {n}C | {n_trucks}T+{n_drones}D | {end}'
        plot_routes(model, result, title,
                    os.path.join(results_dir, f'alns_w6_{exp_key}.png'))

        all_results.append(result_dict)

    # ── Save JSON ──
    json_path = os.path.join(results_dir, 'alns_v222_results.json')
    with open(json_path, 'w') as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nSaved: {json_path}")

    # ── Save summary markdown ──
    summary = "# PACO+ALNS v2.2 Results\n\n"
    ...
    md_path = os.path.join(results_dir, 'alns_v222_summary.md')
