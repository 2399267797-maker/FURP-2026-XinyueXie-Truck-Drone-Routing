...
class CollaborativePACOALNS:
    ...
    def _repair_capacity(self, routes: List[Route]) -> List[Route]:
        ...
        total_capacity = len(routes) * self.truck_capacity
        # Fleet-level lower bound: no inter-route repair can fix an infeasible fleet.
        if total_demand > total_capacity + 1e-6:
            return routes

        loads = [float(sum(self.demands[c] for c in r.customers)) for r in routes]
        max_passes = max(30, len(routes) * 10, self.n_customers * 2)
        cap = self.truck_capacity

        def _mission_sets(r):
            custs, poss = set(), set()
            for m in r.drone_missions:
                custs.update(m.customer_ids)
                poss.add(m.launch_point)
                poss.add(m.return_point)
            return custs, poss

        for _ in range(max_passes):
            overloaded_idx = [i for i, r in enumerate(routes) if loads[i] > cap + 1e-6]
            if not overloaded_idx:
                break

            # 1) move customers from overloaded routes to routes with spare capacity
            moved = False
            for src_idx in sorted(overloaded_idx, key=lambda i: -loads[i]):
                src = routes[src_idx]
                if loads[src_idx] <= cap + 1e-6:
                    continue
                targets = sorted((i for i in range(len(routes)) if i != src_idx),
                                 key=lambda i: loads[i])
                src_mission_cust, src_mission_pos = _mission_sets(src)
                for pos, cust in enumerate(src.customers):
                    if cust in src_mission_cust or pos in src_mission_pos:
                        continue
                    for t_idx in targets:
                        if loads[t_idx] + self.demands[cust] <= cap + 1e-6:
                            src.customers.pop(pos)
                            routes[t_idx].customers.append(cust)
                            for m in src.drone_missions:
                                if m.launch_point > pos:
                                    m.launch_point -= 1
                                if m.return_point > pos:
                                    m.return_point -= 1
                            loads[src_idx] -= self.demands[cust]
                            loads[t_idx] += self.demands[cust]
                            src._timeline_dirty = routes[t_idx]._timeline_dirty = True
                            moved = True
                            break
                    if moved:
                        break
            ...
            # 2) convert overloaded customers into drone missions
            converted = False
            for r_idx in overloaded_idx:
                r = routes[r_idx]
                pos = 0
                while pos < len(r.customers) - 1:
                    if loads[r_idx] <= cap + 1e-6:
                        break
                    j = r.customers[pos]
                    if self.demands[j] > self.drone_capacity + 1e-6:
                        pos += 1
                    ...
                    for gap in range(1, min(self.max_drone_gap + 1, len(r.customers) - pos)):
                        ...
                        if self.dist_matrix[i_n, j + 1] + self.dist_matrix[j + 1, k_n] > self.drone_range + 1e-6:
                            continue
            ...
            # 3) swap customers between overloaded and lighter routes
            swapped = False
            src_idx = overloaded_idx[0]
            src = routes[src_idx]
            targets = sorted((i for i in range(len(routes)) if i != src_idx),
                             key=lambda i: loads[i])
            for t_idx in targets:
                if swapped:
                    break
                target = routes[t_idx]
                t_custs, t_poss = _mission_sets(target)
                s_custs, s_poss = _mission_sets(src)
                for tpos, tcust in enumerate(target.customers):
                    if tcust in t_custs or tpos in t_poss:
                        continue
                    target.customers.pop(tpos)
                    for m in target.drone_missions:
                        if m.launch_point > tpos:
                            m.launch_point -= 1
                        if m.return_point > tpos:
                            m.return_point -= 1
                    for spos, scust in enumerate(src.customers):
                        if scust == tcust or scust in s_custs or spos in s_poss:
                            continue
                        if loads[t_idx] - self.demands[tcust] + self.demands[scust] <= cap + 1e-6:
                            src.customers.pop(spos)
                            target.customers.append(scust)
                            for m in src.drone_missions:
                                if m.launch_point > spos:
                                    m.launch_point -= 1
                                if m.return_point > spos:
                                    m.return_point -= 1
                            src.customers.append(tcust)
                            loads[src_idx] += self.demands[tcust] - self.demands[scust]
                            loads[t_idx] += self.demands[scust] - self.demands[tcust]
